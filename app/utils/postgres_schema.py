...
def ensure_app_schema(conn: PgConnection) -> None:
    with conn.cursor() as cur:
        cur.execute(APP_SCHEMA_SQL)


def ensure_cards_index_schema(conn: PgConnection) -> None:
    ensure_app_schema(conn)


def ensure_prices_limitless_schema(conn: PgConnection) -> None:
    ensure_app_schema(conn)


def ensure_ebay_schema(conn: PgConnection) -> None:
    ensure_app_schema(conn)


def ensure_hareruya_schema(conn: PgConnection) -> None:
    ensure_app_schema(conn)


def ensure_cardrush_schema(conn: PgConnection) -> None:
    ensure_app_schema(conn)
